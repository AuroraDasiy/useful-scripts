...
#自动化脚本按钮
    scipts_button=ttk.Button(frame,text='文件批处理')
    scipts_button.pack(side=tk.LEFT)
    frame.pack(fill=tk.X)
    fm3=ttk.Frame(root);fm3.pack(fill=tk.BOTH)
    tr_lb1=ttk.Label(fm3,text='计算器',font="Serif 18 bold");tr_lb1.pack()
#翻译容器
...
# 创建数字按钮
    for i in range(3):
        for j in range(3):
            btn_id = str(1 + i * 3 + j)
            ttk.Button(fm1, text=btn_id, width=11, command=lambda s=btn_id: click(s)).grid(row=i + 1, column=j, padx=2, pady=2)
    ...
    fm2=ttk.Frame(root);fm2.pack(fill=tk.BOTH)
    tr_lb=ttk.Label(fm2,text='翻译',font="Serif 18 bold");tr_lb.pack()
#翻译
...
def creat_root(title):
    global root
    root=tk.Tk();root.title(title);root.geometry('350x500+700+300')
    creat_frame(root)
    root.mainloop()
...
def rename_file_extensions(directory, old_extension, new_extension):
    ...
    :param old_extension: 要修改的旧扩展名（例如：'.txt'）
    :param new_extension: 新的扩展名（例如：'.md'）
    """
